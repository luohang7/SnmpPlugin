...
class SNMPTrapReceiver:
    """SNMP Trap 接收器，监听指定端口接收SNMP Trap消息"""

    def __init__(self, port: int = 162, host: str = '0.0.0.0'):
        self.port = port
        self.host = host
        self.running = False
        self.socket = None
    ...
    async def start(self):
        """启动SNMP Trap监听服务"""
        if self.running:
            self.logger.warning("SNMP Trap服务已在运行中")
            return
        ...
        try:
            ...
            self.logger.info(f"🔌 SNMP Trap监听服务已启动")
            self.logger.info(f"🌐 监听地址: {self.host}:{self.port}")
            self.logger.info(f"📡 等待接收SNMP Trap消息...")

            # 直接启动监听循环，不创建新任务
            await self._receive_traps()

        except Exception as e:
            print(f"❌ 启动SNMP Trap服务失败: {e}")
            self.logger.error(f"启动SNMP Trap服务失败: {e}")
            self.running = False
            if self.socket:
                self.socket.close()
                self.socket = None
    ...
    async def stop(self):
        ...
        self.running = False
        self.logger.info("🛑 正在停止SNMP Trap监听服务...")
        ...
        self.logger.info(f"✅ SNMP Trap监听服务已停止，共接收 {self.received_traps} 条Trap")

    async def _receive_traps(self):
        """接收SNMP Trap消息的主循环"""
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    self.logger.info(f"📨 接收到来自 {addr[0]}:{addr[1]} 的数据包")

                    # 解析SNMP Trap
                    trap_info = await self._parse_snmp_trap(data, addr)
                    if trap_info:
                        self.received_traps += 1
                        print(f"🚨 SNMP Trap #{self.received_traps} 已解析")
                        self.logger.info(f"🚨 SNMP Trap #{self.received_traps} 已解析")

                        # 调用回调函数处理trap
                        if self.message_callback:
                            await self._safe_callback(trap_info)
                ...
                if self.running:  # 只有在服务还在运行时才记录错误
                    ...
                    self.logger.error(f"SNMP Trap监听循环出错: {e}")
                await asyncio.sleep(0.1)  # 短暂休眠后继续

    async def _parse_snmp_trap(self, data: bytes, addr: tuple) -> Optional[Dict[str, Any]]:
        """解析SNMP Trap数据（简化版本）"""
        try:
            # 基本的解析逻辑（实际项目中需要完整的SNMP协议解析）
            trap_info = {
            ...
            self.logger.debug(f"🔍 SNMP Trap解析结果: {trap_info}")
            return trap_info

        except Exception as e:
            self.logger.error(f"解析SNMP Trap时出错: {e}")
            return None
    ...
    def _extract_readable_text(self, data: bytes) -> Optional[str]:
        ...
        try:
            ...
            self.logger.debug(f"提取可读文本时出错: {e}")
    ...
    async def _safe_callback(self, trap_info: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.error(f"执行SNMP Trap回调函数时出错: {e}")
